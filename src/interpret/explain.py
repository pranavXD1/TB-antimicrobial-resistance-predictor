...
Two things clinicians and reviewers want from an AMR predictor:
  1. GLOBAL: which mutations is the model actually using for each drug? If the
     ...
     confounded.
  2. LOCAL: for *this* isolate, WHY did you predict resistant?

We compute exact TreeSHAP via XGBoost's own `pred_contribs` (booster.predict),
which avoids shap<->xgboost version mismatches in TreeExplainer's model parser
(newer XGBoost stores base_score as "[5E-1]", which older shap can't parse). The
resulting SHAP arrays are handed to shap only for plotting.
"""
...
def _tree_shap(model, X: pd.DataFrame) -> np.ndarray:
    """Exact per-feature TreeSHAP (n_samples x n_features) via XGBoost native
    pred_contribs. Drops the final bias/base column."""
